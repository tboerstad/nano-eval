...
@click.option(
    "--tasks",
    required=True,
    help="Comma-separated task names (e.g. gsm8k_cot_llama,chartqa)",
)
@click.option(
    "--base-url",
    required=True,
...
@click.option(
    "--model",
    default=None,
    help="Model name, auto-detected if API serves only one",
)
...
@click.option("--max-samples", type=int, default=None, help="Max samples per task")
@click.option(
    "--output-path",
    type=click.Path(),
    default=None,
